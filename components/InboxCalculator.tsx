...
export default function InboxCalculator() {
  ...
  const [googleResults, setGoogleResults] = useState<CalculationResults | null>(null);
  ...
  useEffect(() => {
    setValue('ourOffer.costPerDomain', provider === 'outlook' ? 30 : 30);
  }, [provider, setValue]);
  ...
  const onSubmit = (data: CalculatorFormSchema) => {
    ...
    const calculatedResults = calculateSavings(data.currentCosts, data.ourOffer, 'outlook');
  ...
  const googleOfferUseExistingDomains = watchGoogle('ourOffer.useExistingDomains');
  ...
  const googleCurrentTotal = googleCurrentInboxCost + googleCurrentDomainCost + watchGoogle('currentCosts.emailSequencerCost');
  const googleCurrentSendsPerInbox = googleDailyVolume / (googleDomains * googleInboxesPerDomain);

  const googleOfferSendsPerDomain = googleOfferInboxesPerDomain * googleOfferSendsPerInbox;
  const googleOfferDomainsNeeded = Math.ceil(googleOfferDailyVolume / googleOfferSendsPerDomain);
  const googleOfferInboxesNeeded = googleOfferDomainsNeeded * googleOfferInboxesPerDomain;
  const googleOfferInboxCost = googleOfferInboxesNeeded * googleOfferCostPerInbox;
  const googleOfferDomainCost = googleOfferUseExistingDomains ? 0 : googleOfferDomainsNeeded * googleOfferCostForDomains;
  const googleOfferTotal = googleOfferInboxCost + googleOfferDomainCost + googleOfferSequencerCost;

  const onSubmitGoogle = (data: CalculatorFormSchema) => {
    const calculatedResults = calculateSavings(data.currentCosts, data.ourOffer, 'google');
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="flex justify-center py-4">
        <Tabs value={provider} onValueChange={val => val && setProvider(val as 'outlook' | 'google')}>
          <TabsList>
          ...
          <TabsContent value="outlook">
            ...
            <main className="max-w-7xl mx-auto p-4 sm:p-6">
              {/* If Google is selected, show placeholder for now */}
              {provider === 'google' ? (
                <div className="flex flex-col items-center justify-center min-h-[300px]">
                  <h2 className="text-xl font-semibold mb-2">Google Inbox Calculator Coming Soon</h2>
                  <p className="text-muted-foreground">Please select Outlook to use the calculator for now.</p>
                </div>
              ) : (
                <div className={cn(
                  "grid gap-6",
    ...
    Start Saving
          ...
          <TabsContent value="google">
            <div className="max-w-7xl mx-auto p-4 sm:p-6">
              <header className="bg-card border-b px-4 sm:px-6 py-4 mb-6 rounded-xl">
                <div className="max-w-4xl mx-auto">
                  <h1 className="text-xl sm:text-2xl font-semibold">Google Inbox Cost Calculator</h1>
                  <p className="text-muted-foreground text-sm sm:text-base">
                    Calculate your Google email infrastructure costs and see your savings with our offer.
                  </p>
                </div>
              </header>
              <form onSubmit={handleSubmitGoogle(onSubmitGoogle)}>
                <div className="grid gap-6 grid-cols-1 md:grid-cols-2">
                  ...
                  <Card className="border-2 border-blue-100 shadow-lg hover:shadow-xl transition-all duration-300">
                    <CardHeader className="bg-gradient-to-br from-blue-50 to-indigo-50 border-b border-blue-200">
                      ...
                      <CardDescription className="text-blue-600">Enter your current Google email infrastructure costs</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-4 pt-6">
                      <div>
                        <Label htmlFor="currentCosts.emailSequencerCost" className="text-sm font-medium text-gray-700">Email sequencer cost (per month)</Label>
                        <Input
                          ...
                          type="number"
                          step="0.01"
                          {...registerGoogle('currentCosts.emailSequencerCost', { valueAsNumber: true })}
                          placeholder="97"
                          className="mt-1 border-gray-300 focus:border-blue-500 focus:ring-blue-500"
                        />
                      ...
                      <div>
                        ...
                        <Input
                          id="currentCosts.dailyEmailVolume"
                          type="number"
                          {...registerGoogle('currentCosts.dailyEmailVolume', { valueAsNumber: true })}
                          placeholder="2000"
                          className="mt-1 border-gray-300 focus:border-blue-500 focus:ring-blue-500"
                        />
                      ...
                      <div>
                        <Label htmlFor="currentCosts.inboxesPerDomain" className="text-sm font-medium text-gray-700">How many inboxes do you have on each domain?</Label>
                        <Input
                          id="currentCosts.inboxesPerDomain"
                          type="number"
                          {...registerGoogle('currentCosts.inboxesPerDomain', { valueAsNumber: true })}
                          placeholder="3"
                          className="mt-1 border-gray-300 focus:border-blue-500 focus:ring-blue-500"
                        />
                      ...
                      <div>
                        ...
                        <Input
                          id="currentCosts.numberOfDomains"
                          type="number"
                          {...registerGoogle('currentCosts.numberOfDomains', { valueAsNumber: true })}
                          placeholder="23"
                          className="mt-1 border-gray-300 focus:border-blue-500 focus:ring-blue-500"
                        />
                      ...
                      <div>
                        <Label htmlFor="currentCosts.costPerInbox" className="text-sm font-medium text-gray-700">Current cost per inbox per month?</Label>
                        <Input
                          id="currentCosts.costPerInbox"
                          type="number"
                          step="0.01"
                          {...registerGoogle('currentCosts.costPerInbox', { valueAsNumber: true })}
                          placeholder="7.2"
                          className="mt-1 border-gray-300 focus:border-blue-500 focus:ring-blue-500"
                        />
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          type="number"
                          step="0.01"
                          {...registerGoogle('currentCosts.domainCost', { valueAsNumber: true })}
                          placeholder="11.99"
                          className="mt-1 border-gray-300 focus:border-blue-500 focus:ring-blue-500"
                        />
                      ...
                      <div className="pt-2">
                        <div className="text-xs text-gray-500">Current sending volume per inbox: <span className="font-semibold text-blue-700">{isFinite(googleCurrentSendsPerInbox) ? googleCurrentSendsPerInbox.toFixed(2) : '-'}</span></div>
                        <div className="text-xs text-gray-500">Current total inboxes: <span className="font-semibold text-blue-700">{googleCurrentInboxes}</span></div>
                        <div className="text-xs text-gray-500">Current total inbox cost: <span className="font-semibold text-blue-700">{formatCurrency(googleCurrentInboxCost)}</span></div>
                        <div className="text-xs text-gray-500">Current total domain cost: <span className="font-semibold text-blue-700">{formatCurrency(googleCurrentDomainCost)}</span></div>
                        <div className="text-xs text-gray-500">Current total monthly cost: <span className="font-semibold text-blue-700">{formatCurrency(googleCurrentTotal)}</span></div>
                      </div>
                    </CardContent>
                  </Card>
                  {/* Our Offer */}
                  <Card className="border-2 border-green-100 shadow-lg hover:shadow-xl transition-all duration-300">
                    ...
                    <CardContent className="space-y-4 pt-6">
                      <div>
                        <Label htmlFor="ourOffer.emailSequencerCost" className="text-sm font-medium text-gray-700">Email sequencer cost (per month)</Label>
                        <Input
                          id="ourOffer.emailSequencerCost"
                          type="number"
                          step="0.01"
                          {...registerGoogle('ourOffer.emailSequencerCost', { valueAsNumber: true })}
                          placeholder="70"
                          className="mt-1 border-gray-300 focus:border-green-500 focus:ring-green-500"
                        />
                        {errorsGoogle.ourOffer?.emailSequencerCost && (
                          <p className="text-red-600 text-sm mt-1">
                            {errorsGoogle.ourOffer.emailSequencerCost.message}
                          </p>
                        )}
                      </div>
                      <div>
                        ...
                        <Input
                          id="ourOffer.desiredDailyVolume"
                          type="number"
                          {...registerGoogle('ourOffer.desiredDailyVolume', { valueAsNumber: true })}
                          placeholder="2000"
                          className="mt-1 border-gray-300 focus:border-green-500 focus:ring-green-500"
                        />
                        {errorsGoogle.ourOffer?.desiredDailyVolume && (
                          <p className="text-red-600 text-sm mt-1">
                            {errorsGoogle.ourOffer.desiredDailyVolume.message}
                          </p>
                        )}
                      </div>
                      <div>
                        <Label htmlFor="ourOffer.inboxesPerDomain" className="text-sm font-medium text-gray-700">How many inboxes per domain do you want?</Label>
                        <Input
                          id="ourOffer.inboxesPerDomain"
                          type="number"
                          {...registerGoogle('ourOffer.inboxesPerDomain', { valueAsNumber: true })}
                          placeholder="3"
                          className="mt-1 border-gray-300 focus:border-green-500 focus:ring-green-500"
                        />
                        {errorsGoogle.ourOffer?.inboxesPerDomain && (
                          <p className="text-red-600 text-sm mt-1">
                            {errorsGoogle.ourOffer.inboxesPerDomain.message}
                          </p>
                        )}
                      </div>
                      <div>
                        <Label htmlFor="ourOffer.sendsPerInbox" className="text-sm font-medium text-gray-700">Number of sends per inbox (editable)</Label>
                        <Input
                          id="ourOffer.sendsPerInbox"
                          type="number"
                          {...registerGoogle('ourOffer.sendsPerInbox', { valueAsNumber: true })}
                          placeholder="30"
                          className="mt-1 border-gray-300 focus:border-green-500 focus:ring-green-500"
                        />
                        {errorsGoogle.ourOffer?.sendsPerInbox && (
                          <p className="text-red-600 text-sm mt-1">
                            {errorsGoogle.ourOffer.sendsPerInbox.message}
                          </p>
                        )}
                      </div>
                      <div>
                        <Label htmlFor="ourOffer.costPerInbox" className="text-sm font-medium text-gray-700">Our cost per inbox per month</Label>
                        <Input
                          id="ourOffer.costPerInbox"
                          type="number"
                          step="0.01"
                          {...registerGoogle('ourOffer.costPerInbox', { valueAsNumber: true })}
                          placeholder="3.5"
                          className="mt-1 border-gray-300 focus:border-green-500 focus:ring-green-500"
                        />
                        {errorsGoogle.ourOffer?.costPerInbox && (
                          <p className="text-red-600 text-sm mt-1">
                            {errorsGoogle.ourOffer.costPerInbox.message}
                          </p>
                        )}
                      </div>
                      <div>
                        ...
                        <Input
                          ...
                          type="number"
                          step="0.01"
                          {...registerGoogle('ourOffer.costForDomains', { valueAsNumber: true })}
                          placeholder="11.99"
                          className="mt-1 border-gray-300 focus:border-green-500 focus:ring-green-500"
                        />
                        {errorsGoogle.ourOffer?.costForDomains && (
                          <p className="text-red-600 text-sm mt-1">
                            {errorsGoogle.ourOffer.costForDomains.message}
                          </p>
                        )}
                      </div>
                      <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg border border-gray-200">
                        <Switch
                          id="ourOffer.useExistingDomains"
                          checked={watchGoogle('ourOffer.useExistingDomains')}
                          onCheckedChange={checked => {
                            setValueGoogle('ourOffer.useExistingDomains', checked);
                            setValueGoogle('ourOffer.costForDomains', checked ? 0 : watchGoogle('ourOffer.costForDomains'));
                          }}
                        />
                        <Label htmlFor="ourOffer.useExistingDomains" className="text-sm font-medium text-gray-700">Bring your existing domains</Label>
                      </div>
                      <div className="pt-2">
                        <div className="text-xs text-gray-500">Domains needed: <span className="font-semibold text-green-700">{googleOfferDomainsNeeded}</span></div>
                        <div className="text-xs text-gray-500">Inboxes needed: <span className="font-semibold text-green-700">{googleOfferInboxesNeeded}</span></div>
                        <div className="text-xs text-gray-500">Inbox cost: <span className="font-semibold text-green-700">{formatCurrency(googleOfferInboxCost)}</span></div>
                        <div className="text-xs text-gray-500">Domain cost: <span className="font-semibold text-green-700">{formatCurrency(googleOfferDomainCost)}</span></div>
                        <div className="text-xs text-gray-500">Total monthly cost: <span className="font-semibold text-green-700">{formatCurrency(googleOfferTotal)}</span></div>
                      </div>
                    </CardContent>
                    <CardFooter className="bg-gradient-to-r from-green-50 to-emerald-50 border-t border-green-200">
                      <Button type="submit" className="w-full bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-white font-semibold py-3 text-lg shadow-lg hover:shadow-xl transition-all duration-300">
                        <TrendingUp className="h-5 w-5 mr-2" />
                        Calculate Savings
                      </Button>
                    </CardFooter>
                ...
                <div className="space-y-6 mt-8">
                  ...
                  <Card className="bg-gradient-to-br from-green-500 via-emerald-500 to-teal-600 border-0 shadow-2xl overflow-hidden">
                    <div className="relative">
                      ...
                      <CardHeader className="text-center relative z-10 pb-4">
                        ...
                        <CardDescription className="text-lg sm:text-2xl text-white/90 font-semibold mb-4">
                          Monthly Savings • {formatPercentage(googleResults.totalSavingsPercentage)}
                        </CardDescription>
                      </CardHeader>
                      <CardContent className="relative z-10">
                        <div className="text-center">
                  ...
                  <Card className="border-2 border-green-200 shadow-lg">
                    ...
                    <CardContent className="space-y-6 pt-6">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="space-y-4">
                          <div className="text-center">
                            <h3 className="text-lg font-semibold text-red-700 mb-2">Current Costs</h3>
                            <div className="text-3xl font-bold text-red-600">
                              {formatCurrency(googleResults.currentTotalWithDomains - googleResults.currentDomainCost)}
                            </div>
                            <p className="text-sm text-gray-600">per month</p>
                          </div>
                          <div className="space-y-3">
                            <div className="flex justify-between items-center p-3 bg-red-50 rounded-lg border border-red-200">
                              <span className="text-sm font-medium text-gray-700">Email Sequencer</span>
                              <span className="font-semibold text-red-600">{formatCurrency(watchGoogle('currentCosts.emailSequencerCost'))}</span>
                            </div>
                            <div className="flex justify-between items-center p-3 bg-red-50 rounded-lg border border-red-200">
                              <span className="text-sm font-medium text-gray-700">Inbox Costs</span>
                              <span className="font-semibold text-red-600">{formatCurrency(googleCurrentInboxCost)}</span>
                            </div>
                            <div className="flex justify-center items-center">
                              <span className="text-4xl font-extrabold text-gray-700">+</span>
                            </div>
                            <div className="flex justify-between items-center p-3 bg-red-50 rounded-lg border border-red-200">
                              <span className="text-sm font-medium text-gray-700">Domain Costs</span>
                              <span className="font-semibold text-red-600">{formatCurrency(googleCurrentDomainCost)}</span>
                            </div>
                          </div>
                        </div>
                        {/* After */}
                        <div className="space-y-4">
                          ...
                          <div className="space-y-3">
                            <div className="flex justify-between items-center p-3 bg-green-50 rounded-lg border border-green-200">
                              <span className="text-sm font-medium text-gray-700">Our Email Sequencer</span>
                              <span className="font-semibold text-green-600">{formatCurrency(watchGoogle('ourOffer.emailSequencerCost'))}</span>
                            </div>
                            <div className="flex justify-between items-center p-3 bg-green-50 rounded-lg border border-green-200">
                              <span className="text-sm font-medium text-gray-700">Our Inbox Costs</span>
                              <span className="font-semibold text-green-600">{formatCurrency(googleOfferInboxCost)}</span>
                            </div>
                            <div className="flex justify-center items-center">
                              <span className="text-4xl font-extrabold text-gray-700">+</span>
                            </div>
                            <div className="flex justify-between items-center p-3 bg-green-50 rounded-lg border border-green-200">
                              <span className="text-sm font-medium text-gray-700">Your Domain Costs</span>
                              <span className="font-semibold text-green-600">{formatCurrency(googleOfferDomainCost)}</span>
                            </div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="bg-gradient-to-r from-red-50 via-yellow-50 to-green-50 rounded-xl p-6 border-2 border-gray-200">
                        <div className="text-center flex flex-col justify-center items-center">
                          <div className="text-lg font-semibold text-gray-800 mb-2">Ready to start saving {formatCurrency(googleResults.totalSavings)} per month?</div>
